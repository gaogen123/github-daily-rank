...
export function createRuntime({
  ...
  app.post('/api/search', async (request, response) => {
    ...
    const minStars = Number(request.body?.minStars ?? 0);
    const maxStars = request.body?.maxStars == null ? null : Number(request.body.maxStars);
    if (!Number.isFinite(minStars) || minStars < 0 || (maxStars !== null && (!Number.isFinite(maxStars) || maxStars < minStars))) {
      return response.status(400).json({ code: 'INVALID_RANGE', message: 'Star 范围无效' });
    }
  ...
  function startScheduler() {
    if (environment.ENABLE_VECTOR_SCHEDULER !== 'true') {
    ...
    const expression = environment.VECTOR_INDEX_CRON || '30 10 * * *';
